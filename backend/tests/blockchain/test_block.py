...
from backend.blockchain.block3 import Block, GENESIS_DATA
...
def test_quickly_mined_block():
    ...
    '''this mine block was created in the smallest timeframe possible since its last block
    so we can reasonablly assume that the block is going to be mined pretty quicky'''
    assert mined_block.difficulty == last_block.difficulty + 1

def test_slowly_mined_block():
    last_block = Block.mine_block(Block.genesis(), 'foo')
    time.sleep(MINE_RATE / SECONDS) # sleep takes seconds as its value
    mined_block = Block.mine_block(last_block, 'bar')
    assert mined_block.difficulty == last_block.difficulty - 1
...
def test_is_valid_block_last_hash(last_block, block):
    ...
    with pytest.raises(Exception, match = 'The block last_hash must correct'):
        Block.is_valid_block(last_block, block)
